...
class pybindIETFJSONEncoder(pybindJSONEncoder):
  @staticmethod
  def generate_element(obj, parent_namespace=None, flt=False, ignore_opdata=False, ignore_conf_leaves=False,
                       save_metadata_to=None):
    """
    ...
    if generated_by == "YANGListType":
      return [pybindIETFJSONEncoder.generate_element(i, flt=flt, ignore_opdata=ignore_opdata,
                                                     ignore_conf_leaves=ignore_conf_leaves,
                                                     save_metadata_to=save_metadata_to) for i in
                                                            obj.itervalues()]
    ...
    for element_name in obj._pyangbind_elements:
      ...
      if generated_by == "container":
        d[yname] = pybindIETFJSONEncoder.generate_element(element,
                      ...
                      ignore_conf_leaves=ignore_conf_leaves, save_metadata_to=save_metadata_to)
        if not len(d[yname]):
          del d[yname]
...
#        print save_metadata_to,'<<from inside'
        if save_metadata_to is not None:
          if element._yang_type == 'enumeration':
#             print element.__dict__
             _ytype = element._yang_type
             _ytype = [element._path(), 'enumeration',  element._default, element._base_type._enumeration_dict, None]
          else:
             _ytype = [element._path(), element._yang_type, element._default, None, None]
          save_metadata_to.append(_ytype) 
        if ignore_opdata and element._is_config is False:
          pass
        elif ignore_conf_leaves and element._is_config and element._is_leaf and element._is_keyval is False:
          pass
